...
case "${1:-}" in
    ...
    *)
        ...
        echo ""
        echo "用法:"
        echo "  video-toolkit all     <feature>    全流程: 字幕→AI配音→合成"
        echo "  video-toolkit srt     <feature>    仅提取字幕（需要 Whisper）"
        echo "  video-toolkit dub     <feature>    仅生成 AI 配音（试听用）"
        echo "  video-toolkit mix     <feature>    仅合成视频"
        echo ""
        echo "  video-toolkit trans   <feature>    翻译字幕: 中文→英文 (DeepSeek)"
        echo "  video-toolkit en      <feature>    英文全流程: 翻译→配音→合成"
