...
新建视频项目时，把 [RULE.md](RULE.md) 复制一份到项目根目录，按产品品牌色/字体/语音改写"品牌与色调"一节。
里面规定了每个 feature 该产出什么、录制/演示规范、录制前防泄露检查清单、色调一致性要求，以及不能做什么——
写 feature 之前先过一遍，后续 feature 也直接读这份文件确定范围和限制。
